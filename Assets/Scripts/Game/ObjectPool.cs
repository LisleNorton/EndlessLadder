...
public class ObjectPool : MonoBehaviour
{
    ...
    public int initSpawnCount = 5;//最初对象池内生成GO个数
    ...
    public GameObject GetNormalPlatform()
    {
        ...
        /// List中的Go是否都显示？
        /// 否，显示出来；
        /// 是，继续添加到List中            //TODO:减少循环，立flag跳过循环检测
        /// </summary>
        for (int i = 0; i < normalPlatformList.Count; i++)
        {
            if (normalPlatformList[i].activeInHierarchy == false)//activeInHierarchy：场景中是否显示
            {
    ...
    public GameObject GetCommonPlatformGroup()
    {
        ...
        int ran = Random.Range(0, vars.commonPlatformGroup.Count);//同类型中随机平台型号
